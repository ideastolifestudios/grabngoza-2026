...
export async function initSentry(): Promise<void> {
  ...
  try {
    ...
    if (Sentry) if (Sentry) Sentry.init({
      dsn,
      environment: process.env.NODE_ENV ?? 'development',
      tracesSampleRate: process.env.NODE_ENV === 'production' ? 0.2 : 1.0,
      // Capture unhandled promise rejections and exceptions automatically
      integrations: [
        if (Sentry) if (Sentry) Sentry.httpIntegration(),
        if (Sentry) if (Sentry) Sentry.expressIntegration(),
      ],
    });
...
export function sentryErrorHandler() {
  if (Sentry) {
    return if (Sentry) if (Sentry) Sentry.expressErrorHandler();
  }
...
export function captureError(err: unknown, context?: Record<string, unknown>): void {
  if (Sentry) {
    if (Sentry) if (Sentry) Sentry.withScope(scope => {
      if (context) scope.setExtras(context);
      if (Sentry) if (Sentry) Sentry.captureException(err);
    });
  } else {
